...
const response = await axios.post(
     `http://localhost:7777/adoptions/${id}/apply`,
     {},                                   // aucun corps pour cette requête
     {        headers: { Authorization: `Bearer ${token}` }
      }
   );

    setHasApplied(true);
    setApplyMessage("Vous avez postulé avec succès !");
  } catch (error) {
    setApplyMessage(error.response?.data?.message || "Erreur lors de la postulation.");
  }
};
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className="flex-1 flex items-center justify-center bg-teal-600 hover:bg-teal-700 text-white py-3 px-6 rounded-lg font-medium transition duration-200"
                >
                  <FaPhone className="mr-3" />
                  {showContact ? 'Masquer les contacts' : 'Afficher les contacts'}
                </button>
       {annonce.type === "adoption" &&
  annonce.user?._id !== JSON.parse(localStorage.getItem("userData"))?.id && (
    <button
      onClick={handleApply}
      disabled={hasApplied}
    ...
    >
      {hasApplied ? "Déjà postulé" : "Postuler à cette annonce"}
    </button>
)}
                ...
                {annonce.user?._id === JSON.parse(localStorage.getItem("userData"))?.id && (
                  <Link
                    to={`/annonces/${id}/edit`}
                    className="flex-1 flex items-center justify-center bg-white border border-gray-300 hover:bg-gray-50 text-gray-800 py-3 px-6 rounded-lg font-medium transition duration-200"
                  >
                    <FaEdit className="mr-3" /> Modifier l'annonce
                  </Link>
                )}
              </div>
